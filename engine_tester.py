...
def get_player_action(player, highest_bet):
    try:
        ...
        current_bet = int(bet_text.replace(",", "")) if bet_text else 0
        previous_bet = int(previous_bets.get(name, "0").replace(",", "")) if name in previous_bets else 0
        delta = current_bet - previous_bet

        if current_bet == 0:
            return "Check" if highest_bet == 0 else "Fold"

        if current_bet < highest_bet:
            return f"Call {current_bet}"
        elif current_bet == highest_bet:
            if delta > 0:
                return f"Call {current_bet}"
            else:
                return "Check"  # Already matched
        else:
            return f"Raise to {current_bet}"

    except Exception as e:
        print(f"[!] Error in get_player_action: {e}")
        return "Unknown"
